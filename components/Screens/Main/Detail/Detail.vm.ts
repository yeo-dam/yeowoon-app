...
export default class DetailViewModel extends BaseViewModel {
  private static _Instance: DetailViewModel;
  ...
  static GetInstance(args: ConstructorParameter) {
    if (!DetailViewModel._Instance) {
      DetailViewModel._Instance = new DetailViewModel(args);
    }
    return DetailViewModel._Instance;
  }
  ...
  @action
  load = flow(function* (this: DetailViewModel) {
    try {
